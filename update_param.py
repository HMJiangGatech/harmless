...
#     parameter['alpha'] *= 0
#     parameter['gamma'] *= 0
#     parameter['psi'] = np.zeros((N, N, K))
#     parameter['phi'] = np.zeros((N, N, K))
#     for i,j in enumerate(clustering.labels_):
#         parameter['alpha'][i,j] = 100
...
def update_parameter(data, parameter, hawkes_models, lr, hardgamma=False, verbose=False, gamma_iter=1, phi_iter=1):
    ...
    for _ in range(phi_iter):
        ...
        # print('phi', np.std(np.argmax(parameter['phi'][1,:,:], axis=1)))
        ...
        # print('psi', np.std(np.argmax(parameter['psi'][:,1,:], axis=1)))
        ...
        # num_B = np.sum(phi_psi*np.expand_dims(np.expand_dims(G_matrix, axis=-1), axis=-1) , axis=(0,1))
        # don_B =  np.sum(phi_psi, axis=(0,1))
        num_B = np.einsum('pq,pqg,pqh->gh',G_matrix,parameter['phi'],parameter['psi'])
        don_B = np.einsum('pqg,pqh->gh',parameter['phi'],parameter['psi'])
